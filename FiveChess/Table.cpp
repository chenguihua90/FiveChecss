...
CTable::CTable()
{
    ...
    ::GetPrivateProfileString( _T("Options"), _T("Name"), _T("zhangji"), str, 15, pApp->m_szIni );
    m_strMe = str;
    // 初始化图像列表
...
void CTable::Over()
{
    ...
    if ( Win( 1 - m_color ) )
    {
        ...
        pDlg->MessageBox( _T("您输了！"), _T("失败"), MB_ICONINFORMATION );
        pDlg->GetDlgItem( IDC_BTN_HQ )->EnableWindow( FALSE );
        pDlg->GetDlgItem( IDC_BTN_BACK )->EnableWindow( FALSE );
        pDlg->GetDlgItem( IDC_BTN_LOST )->EnableWindow( FALSE );
        // 如果是网络对战，则生效“重玩”
...
void CTable::Receive()
{
    ...
    case MSG_AGREEDRAW:
        {
            ...
            pApp->m_nDraw++;
            CDialog *pDlg = (CDialog *)GetParent();
            pDlg->MessageBox( _T("对方接受了您的和棋请求。"), _T("和棋"), MB_ICONINFORMATION );
            // 和棋后，使“重玩”菜单生效
            pDlg->GetMenu()->EnableMenuItem( ID_MENU_PLAYAGAIN, MF_ENABLED | MF_BYCOMMAND );
        }
        break;
    case MSG_REFUSEDRAW:
        {
            CDialog *pDlg = (CDialog *)GetParent();
            pDlg->MessageBox( _T("对方拒绝了您的和棋请求。"),
    ...
    case MSG_GIVEUP:
        {
            ...
            pApp->m_nWin++;
            CDialog *pDlg = (CDialog *)GetParent();
            pDlg->MessageBox( _T("对方已经投子认输！"), _T("胜利"), MB_ICONINFORMATION );
            // 禁用各按钮及棋盘
            pDlg->GetDlgItem( IDC_BTN_BACK )->EnableWindow( FALSE );
            pDlg->GetDlgItem( IDC_BTN_HQ )->EnableWindow( FALSE );
            pDlg->GetDlgItem( IDC_BTN_LOST )->EnableWindow( FALSE );
    ...
    case MSG_PLAYAGAIN:
        {
            CDialog *pDlg = (CDialog *)GetParent();
            if ( IDYES == pDlg->MessageBox( _T("对方请求再战一局，接受这个请求吗？\n\n选“否”将断开与他的连接。"),
...
void CTable::OnLButtonUp( UINT nFlags, CPoint point )
{
    ...
    if ( Win( m_color ) )
    {
        ...
        if(m_color==0)
		{
			pDlg->MessageBox( _T("黑棋获得了胜利！"), _T("对弈结果"), MB_ICONINFORMATION );
        }
		else
		{
			pDlg->MessageBox( _T("白棋获得了胜利！"), _T("对弈结果"), MB_ICONINFORMATION );
		}
		pDlg->GetDlgItem( IDC_BTN_HQ )->EnableWindow( FALSE );
        pDlg->GetDlgItem( IDC_BTN_BACK )->EnableWindow( FALSE );
        pDlg->GetDlgItem( IDC_BTN_LOST )->EnableWindow( FALSE );
