...
BEGIN_MESSAGE_MAP(CFiveDlg, CDialog)
	...
	ON_COMMAND(ID_MENU_DOUBLE, OnMenuDouble)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
{
    if ( IDYES == MessageBox( _T("确定要退出？"), _T("lilei"), MB_ICONQUESTION | MB_YESNO | MB_DEFBUTTON2 ) )
        CDialog::OnCancel();
}
...
void CFiveDlg::OnMenuDouble() 
{
	// TODO: Add your command handler code here
	m_Table.SetColor( 1 );
    m_Table.Clear( FALSE );
    m_Table.SetGameMode( 3 );
}
