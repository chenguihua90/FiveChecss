...
// CDoubleGame类的实现部分
//////////////////////////////////////////////////////////////////////////
CDoubleGame::~CDoubleGame()
{
}
void CDoubleGame::Init()
{
}
void CDoubleGame::Win( const STEP& stepSend )
{
}
void CDoubleGame::SendStep( const STEP& stepPut )
{
}
void CDoubleGame::ReceiveMsg( MSGSTRUCT *pMsg )
{
}
void CDoubleGame::Back()
{
    CDialog *pDlg = (CDialog *)AfxGetMainWnd();
    // 使按钮失效
    pDlg->GetDlgItem( IDC_BTN_BACK )->EnableWindow( FALSE );
    ...
    pDlg->GetDlgItem( IDC_BTN_LOST )->EnableWindow( FALSE );
	//保存悔棋步骤
	int x0=m_pTable->old[0];
	int y0=m_pTable->old[1];
	int x1=m_pTable->old[2];
	int y1=m_pTable->old[3];
	m_pTable->m_data[x0][y0]=-1;
	m_pTable->m_data[x1][y1]=-1;
	m_pTable->Invalidate();
    // 设置等待标志
    m_pTable->SetWait( FALSE );
